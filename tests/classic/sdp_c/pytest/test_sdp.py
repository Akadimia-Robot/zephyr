...
async def sdp_discover_no_record(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        # Discover SDP Record
        shell.exec_command("sdp_client discovery 0001")
        found, lines = await wait_for_shell_response(dut, "No SDP Record")
        logger.info(f'{lines}')
        assert found is True


async def sdp_discover_one_record(hci_port, shell, dut, address) -> None:
    logger.info('<<< SDP Discovery ...')
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
...
async def sdp_discover_two_records(hci_port, shell, dut, address) -> None:
    logger.info('<<< SDP Discovery ...')
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
...
async def sdp_discover_multiple_records(hci_port, shell, dut, address) -> None:
    logger.info('<<< SDP Discovery ...')
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
...
class TestSdpServer:
    def test_sdp_discover_no_record(self, shell: Shell, dut: DeviceAdapter, sdp_client_dut):
        """Test case to request SDP records. No SDP record registered."""
        logger.info(f'test_sdp_discover_no_record {sdp_client_dut}')
        hci, iut_address = sdp_client_dut
        asyncio.run(sdp_discover_no_record(hci, shell, dut, iut_address))

    def test_sdp_discover_one_record(self, shell: Shell, dut: DeviceAdapter, sdp_client_dut):
        """Test case to request SDP records. One SDP record registered."""
        logger.info(f'test_sdp_discover_one_record {sdp_client_dut}')
        hci, iut_address = sdp_client_dut
        asyncio.run(sdp_discover_one_record(hci, shell, dut, iut_address))

    def test_sdp_discover_two_records(self, shell: Shell, dut: DeviceAdapter, sdp_client_dut):
        """Test case to request SDP records. Two SDP record registered."""
        logger.info(f'test_sdp_discover_two_records {sdp_client_dut}')
        hci, iut_address = sdp_client_dut
        asyncio.run(sdp_discover_two_records(hci, shell, dut, iut_address))

    def test_sdp_discover_multiple_records(self, shell: Shell, dut: DeviceAdapter, sdp_client_dut):
        """Test case to request SDP records. Multiple SDP record registered."""
        logger.info(f'test_sdp_discover_multiple_records {sdp_client_dut}')
        hci, iut_address = sdp_client_dut
        asyncio.run(sdp_discover_multiple_records(hci, shell, dut, iut_address))
