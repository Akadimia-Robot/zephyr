...
#define NPCM_PINMUX_INIT(node_id, prop)                                                            \
	COND_CODE_1( \
		...
		(NPCM_PINMUX_GROUP_RDATA( \
				NPCM_PINMUX_ID(node_id, id), \
				NPCM_PINMUX_PIN_REG(node_id, pin_reg), \
...
#define NPCM_PIN_PERIPH_PUPD_GROUP_INIT(node_id)                                                   \
	NPCM_PIN_PERIPH_PUPD_GROUP_RDATA(                                                          \
		NPCM_PINMUX_ID(node_id, id), NPCM_PIN_PERIPH_PUPD_SUP(node_id, pupd_sup),          \
		NPCM_PIN_PERIPH_PUPD_REG0(node_id, pupd_reg),                                      \
		NPCM_PIN_PERIPH_PUPD_TYPE0(node_id, pupd_type), NPCM_PIN_REG_NULL, 0)
#define NPCM_PIN_PERIPH_PUPD_GROUP_EXT_INIT(node_id)                                               \
	NPCM_PIN_PERIPH_PUPD_GROUP_RDATA(NPCM_PINMUX_ID(node_id, id),                              \
					 NPCM_PIN_PERIPH_PUPD_SUP(node_id, pupd_sup),              \
					 NPCM_PIN_PERIPH_PUPD_REG0(node_id, pupd_reg),             \
					 NPCM_PIN_PERIPH_PUPD_TYPE0(node_id, pupd_type),           \
					 NPCM_PIN_PERIPH_PUPD_REG1(node_id, pupd_reg),             \
					 NPCM_PIN_PERIPH_PUPD_TYPE1(node_id, pupd_type))

#define NPCM_PIN_PERIPH_PUPD_INIT(node_id, prop)                                                   \
	COND_CODE_1( \
		DT_PROP(node_id, pinmux_group_def), \
		(COND_CODE_1( \
			DT_NODE_HAS_PROP(node_id, pupd_reg), \
			(COND_CODE_1( \
				DT_PROP(node_id, pupd_config_extend), \
...
static const struct npcm_periph_pupd_cfg npcm_periph_pupd_table[] = {
	DT_FOREACH_CHILD_VARGS(DT_DRV_INST(0), NPCM_PIN_PERIPH_PUPD_INIT)};
...
#define NPCM_PIN_LV_INIT(node_id, prop)                                                            \
	COND_CODE_1( \
		DT_PROP(node_id, pinmux_group_def), \
		(COND_CODE_1( \
			DT_NODE_HAS_PROP(node_id, lv_reg), \
			(NPCM_PIN_LV_GROUP_RDATA( \
				NPCM_PINMUX_ID(node_id, id), \
				NPCM_PIN_LV_REG(node_id, lv_reg), \
				NPCM_PIN_LV_SUP(node_id, lv_sup), \
				NPCM_PIN_LV_LOW_LEVEL(node_id, lv_def) \
				) \
			), \
			()) \
		), \
...
static struct npcm_pin_lv_cfg npcm_pin_lv_table[] = {
	DT_FOREACH_CHILD_VARGS(DT_DRV_INST(0), NPCM_PIN_LV_INIT)};
...
/* Local functions */
static void npcm_periph_pinmux_configure(const struct npcm_pinctrl *pin)
{
...
static void npcm_periph_pupd_configure(const struct npcm_pinctrl *pin)
{
...
static void npcm_device_control_configure(const struct npcm_pinctrl *pin)
{
	const uintptr_t scfg_base = npcm_pinctrl_cfg.base_scfg;
...
static void npcm_volt_level_configure(const struct npcm_pinctrl *pin)
{
...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	for (i = 0; i < pin_cnt; i++) {
		...
		case NPCM_PINCTRL_TYPE_LVOL:
			...
			npcm_volt_level_configure(&pins[i]);
			break;
