...
Artificially long but functional example:
   ...
   "kernel.fifo.poll" is one of the test section names in
    ...
    test_plan_report_xor.add_argument("--list-tests", action="store_true",
        ...
        all --testsuite-root arguments. The output is flattened and reports --sub-test names only,
        without their directories.
        Note: in --detailed-test-id mode different sub-tests can share the same test scenario
        identifier (section.subsection) and come from different directories,
        whereas in --no-detaied-test-id mode the output will have no test scenario identifiers.
        """)
    ...
    test_xor_subtest.add_argument(
        ...
        Note: With --no-detailed-test-id use only Ztest identificator without scenario name prefix.
        """)
