 ...
 * @param velocity_in_usteps_per_second The velocity to set in microsteps per second
...
__syscall int stepper_run(const struct device *dev, enum stepper_direction direction,
			  uint32_t velocity_in_usteps_per_second);
				     ...
				     const enum stepper_direction direction, const uint32_t value)
{
	...
	return api->run(dev, direction, value);
}
