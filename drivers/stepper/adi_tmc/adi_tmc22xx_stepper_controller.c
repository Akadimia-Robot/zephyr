...
#include "../step_dir_stepper_controller.h"
					      ...
					      enum stepper_micro_step_resolution micro_step_res)
{
	...
	for (uint8_t i = 0; i < MSX_PIN_STATE_COUNT; i++) {
		if (micro_step_res == config->msx_resolutions[i]) {
			ret = gpio_pin_set_dt(&config->msx_pins[i], i & 0x01);
			if (ret < 0) {
				LOG_ERR("Failed to set MS%d pin", i + 1);
				return ret;
			}

			ret = gpio_pin_set_dt(&config->msx_pins[i], (i & 0x02) >> 1);
			if (ret < 0) {
				LOG_ERR("Failed to set MS%d pin", i + 1);
				return ret;
			}
...
static int tmc22xx_stepper_init(const struct device *dev)
{
	...
	if (config->msx_pins) {
		...
		if (ret < 0) {
			LOG_ERR("Failed to set microstep resolution");
			return ret;
		}
	}
	...
	if (ret < 0) {
		LOG_ERR("Failed to init step dir common stepper");
		return ret;
	}
...
static DEVICE_API(stepper, tmc22xx_stepper_api) = {
	...
	.move = step_dir_stepper_common_move,
	.is_moving = step_dir_stepper_common_is_moving,
	.set_reference_position = step_dir_stepper_common_set_reference_position,
	.get_actual_position = step_dir_stepper_common_get_actual_position,
	.set_target_position = step_dir_stepper_common_set_target_position,
	.set_max_velocity = step_dir_stepper_common_set_max_velocity,
	.enable_constant_velocity_mode = step_dir_stepper_common_enable_constant_velocity_mode,
...
#define TMC22XX_STEPPER_DEFINE(inst, msx_table)                                                    \
	...
	static const struct gpio_dt_spec tmc22xx_stepper_msx_pins_##inst[] = {              \
		DT_INST_FOREACH_PROP_ELEM_SEP(inst, msx_gpios, GPIO_DT_SPEC_GET_BY_IDX, (,)), \
	};                                                                                 \
	BUILD_ASSERT(ARRAY_SIZE(tmc22xx_stepper_msx_pins_##inst) == MSX_PIN_COUNT,                      \
