...
static uint8_t ticker_job_reschedule_in_window(struct ticker_instance *instance)
{
	...
	while (rescheduling) {
		...
		if (IS_ENABLED(CONFIG_BT_TICKER_EXT_SLOT_WINDOW_YIELD) &&
			 ...
			 * ticks_slot so that we do not take the interval of the
			 * colliding ticker.
			 */
		...
		while ((ticker_id_next != TICKER_NULL) &&
			...
			if (((window_start_ticks + ticks_slot) <=
				...
				if (!ticker_resched->ticks_slot ||
					...
					/* Drift to middle of available slot
					 * window, so as to not take the
					 * interval of the slot window duration.
					 */
					if (ext_data->is_drift_in_window) {
						ticks_to_expire +=
							ticks_slot_window / 2U;
					}
				} else {
