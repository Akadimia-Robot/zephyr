...
static uint8_t sdp_discover_func(struct bt_conn *conn, struct bt_sdp_client_result *result)
{
	...
	if ((result->resp_buf == NULL) || (result->resp_buf->len == 0)) {
		if (sdp_record_found) {
...
static int cmd_discovery(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (len == (BT_UUID_SIZE_16 * 2)) {
		...
		hex2bin(argv[1], len, (uint8_t *)&sdp_discover_uuid.u16.val,
			sizeof(sdp_discover_uuid.u16.val));
		sdp_discover.uuid = &sdp_discover_uuid.u16.uuid;
	} else if (len == (BT_UUID_SIZE_32 * 2)) {
		sdp_discover_uuid.u32.uuid.type = BT_UUID_TYPE_32;
		hex2bin(argv[1], len, (uint8_t *)&sdp_discover_uuid.u32.val,
			sizeof(sdp_discover_uuid.u32.val));
...
SHELL_STATIC_SUBCMD_SET_CREATE(sdp_client_cmds,
	SHELL_CMD_ARG(discovery, NULL, "UUID", cmd_discovery, 2, 0),
